...
// MPU6050 accelerometer scaling
#define MPU6050_ACCEL_SCALE_1G    (GRAVITY_MSS / 4096.0f)
...
bool AP_InertialSensor_MPU6050::_init_sensor(void)
{
...
#ifdef MPU6050_DRDY_PIN
    _drdy_pin = hal.gpio->channel(MPU6050_DRDY_PIN);
    ...
    do {
        ...
        if (tries++ > 5) {
            hal.console->print_P(PSTR("failed to boot MPU6050 5 times"));
            return false;
        }
    } while (1);
...
bool AP_InertialSensor_MPU6050::update( void )
{
    ...
    accel *= MPU6050_ACCEL_SCALE_1G / num_samples;
...
/**
 * Return true if the MPU6050 has new data available for reading.
 *
...
/**
 * Timer process to poll for new data from the MPU6050.
 */
...
{
	uint8_t buf[1];
	if (_i2c->readRegister(MPU6050_ADDR, reg, buf) == 0)
		return buf[0];
...
{
    _i2c->writeRegister(MPU6050_ADDR, reg, val);
}
...
bool AP_InertialSensor_MPU6050::_hardware_init()
{
    ...
    if (tries == 5) {
        hal.console->println_P(PSTR("Failed to boot MPU6050 5 times"));
        _i2c_sem->give();
        return false;
    }
   ...
   uint8_t _mpu6050_product_id = _register_read(MPUREG_PRODUCT_ID);
...
{
    hal.console->println_P(PSTR("MPU6050 registers"));
    if (_i2c_sem->take(100)) {
