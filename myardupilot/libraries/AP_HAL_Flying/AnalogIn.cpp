...
#define Flying_VOLTAGE_SCALING		(float)3.3f/4095
...
//    float scaling;
//} pin_scaling[] = {
//    { PC5,   9 * Flying_VOLTAGE_SCALING }, 	// Battary voltage, 9:1 scaling
//    { PB0,  40 * Flying_VOLTAGE_SCALING }		// Battary current, 20:1 scaling
//};

//const uint8_t FlyingAnalogSource::_num_pin_scaling = sizeof(pin_scaling) / sizeof(pin_scaling[0]);
volatile uint16_t FlyingAnalogSource::_ADCConvData_Tab[Flying_INPUT_MAX_CHANNELS] = {0};
uint8_t FlyingAnalogSource::_ADCChannels_Tab[Flying_INPUT_MAX_CHANNELS] = {0};
uint8_t FlyingAnalogSource::_num_adc_channels = 0;

FlyingAnalogSource::FlyingAnalogSource(uint8_t pin) :
    _sum_count(0),
    _sum(0),
    _last_average(0),
    _latest(0),
...
float FlyingAnalogSource::read_average() {
    uint16_t sum;
    uint8_t sum_count;
...
float FlyingAnalogSource::read_latest() {
    return _latest;
}
 ...
 */
//float FlyingAnalogSource::_pin_scaler(void) {
//	if (_pin_scaling_id != -1)
//		return pin_scaling[_pin_scaling_id].scaling;
//	else {
//        hal.console->println(PSTR("Flying::FlyingAnalogIn can't find pin scaling message, you need to check the pin_scaling[] definiton\r\n"));
//		return 0;
//	}
//
//}
 ...
 */
float FlyingAnalogSource::voltage_average(void) {
    return Flying_VOLTAGE_SCALING * read_average();
}

float FlyingAnalogSource::voltage_latest(void) {
    return Flying_VOLTAGE_SCALING * read_latest();
}
 ...
 */
float FlyingAnalogSource::voltage_average_ratiometric(void) {
    return Flying_VOLTAGE_SCALING * read_average();
}

void FlyingAnalogSource::set_pin(uint8_t pin) {
...
uint16_t FlyingAnalogSource::_get_conv_data(void) {
	if(_channel_rank != -1)
...
void FlyingAnalogSource::_register_adc_channel(uint8_t pin) {
...
void FlyingAnalogSource::_unregister_adc_channel() {
//	_pin_scaling_id = -1;
...
void FlyingAnalogSource::set_stop_pin(uint8_t pin) {
    _stop_pin = pin;
}

void FlyingAnalogSource::set_settle_time(uint16_t settle_time_ms) {
 ...
 */
void FlyingAnalogSource::new_sample(uint16_t sample) {
    _sum += sample;
    _latest = sample;
    if (_sum_count >= 15) { // Flying has a 12 bit ADC, so can only sum 16 in a uint16_t
        _sum >>= 1;
        _sum_count = 8;
    } else {
        _sum_count++;
...
void FlyingAnalogSource::_init_adc1(void) {
...
void FlyingAnalogSource::_update_adc1_config() {
...
FlyingAnalogSource* FlyingAnalogIn::_channels[Flying_INPUT_MAX_CHANNELS] = {NULL};
uint8_t FlyingAnalogIn::_num_channels = 0;
uint8_t FlyingAnalogIn::_current_stop_pin_i = 0;

void FlyingAnalogIn::init(void* machtnichts) {
	FlyingAnalogSource::_init_adc1();
	_attach_interrupt(_dma_event);
}

AP_HAL::AnalogSource* FlyingAnalogIn::channel(int16_t pin)
{
	if (pin == ANALOG_INPUT_NONE) {
	    hal.console->println_P(PSTR("FlyingAnalogIn: Register a analog channel without a actual adc channel\r\n"));
	}

    if (_num_channels >= Flying_INPUT_MAX_CHANNELS) {
        hal.console->println(PSTR("Error: Flying::FlyingAnalogIn out of channels\r\n"));
        return NULL;
    }

    FlyingAnalogSource *ch = new FlyingAnalogSource(pin);
    _channels[_num_channels++] = ch;
    return ch;
}


float FlyingAnalogIn::board_voltage(void) {
    return 3.3f;
}
 ...
 */
void FlyingAnalogIn::_next_stop_pin(void)
{
...
void FlyingAnalogIn::_dma_event() {
...
void FlyingAnalogIn::_attach_interrupt(voidFuncPtr callback) {
	if(callback != NULL)
