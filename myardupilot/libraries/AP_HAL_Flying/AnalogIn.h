...
#ifndef __AP_HAL_Flying_ANALOGIN_H__
#define __AP_HAL_Flying_ANALOGIN_H__
...
#define Flying_INPUT_MAX_CHANNELS 		16
#define Flying_VCC_ANALOG_IN_PIN 		PC5

typedef void (*voidFuncPtr)(void);

class Flying::FlyingAnalogSource : public AP_HAL::AnalogSource {
public:
    friend class Flying::FlyingAnalogIn;

    FlyingAnalogSource(uint8_t pin);
    float read_average();
    float read_latest();
...
//    static const uint8_t _num_pin_scaling;
    ...
    static uint8_t _ADCChannels_Tab[Flying_INPUT_MAX_CHANNELS];
    static uint8_t _num_adc_channels;

    volatile static uint16_t _ADCConvData_Tab[Flying_INPUT_MAX_CHANNELS];
...
class Flying::FlyingAnalogIn : public AP_HAL::AnalogIn {
public:
...
private:
    static FlyingAnalogSource* _channels[Flying_INPUT_MAX_CHANNELS];
...
#endif // __AP_HAL_Flying_ANALOGIN_H__
