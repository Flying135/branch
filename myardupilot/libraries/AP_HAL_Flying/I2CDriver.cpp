...
FlyingI2CDriver::FlyingI2CDriver(I2C_TypeDef* i2c, GPIO_TypeDef* port,
...
void FlyingI2CDriver::begin() {
	_i2c_config(_i2c_info);
}

void FlyingI2CDriver::end() {
	/* Disable I2C */
...
void FlyingI2CDriver::setTimeout(uint16_t ms) {
	_timeout  = ms;
}

void FlyingI2CDriver::setHighSpeed(bool active) {
...
uint8_t FlyingI2CDriver::write(uint8_t addr, uint8_t len, uint8_t* data) {
	uint32_t tmpreg = 0;
	uint32_t start;
...
uint8_t FlyingI2CDriver::writeRegister(uint8_t addr, uint8_t reg, uint8_t val) {
	return 	writeRegisters(addr, reg, 1, &val);
}

uint8_t FlyingI2CDriver::writeRegisters(uint8_t addr, uint8_t reg, uint8_t len, uint8_t* data) {
	uint32_t tmpreg = 0;
	uint32_t start;
	bool isTimeout = false;
	/*
...
uint8_t FlyingI2CDriver::read(uint8_t addr, uint8_t len, uint8_t* data) {
	memset(data, 0, len);
...
uint8_t FlyingI2CDriver::readRegister(uint8_t addr, uint8_t reg, uint8_t* data) {
    return readRegisters(addr, reg, 1, data);
}

uint8_t FlyingI2CDriver::readRegisters(uint8_t addr, uint8_t reg, uint8_t len, uint8_t* data) {
	memset(data, 0, len);
...
uint8_t FlyingI2CDriver::lockup_count() {
	return _lockup_count;;
}


void FlyingI2CDriver::_i2c_bus_reset(struct I2C_Info &i2c_info) {
...
void FlyingI2CDriver::_i2c_config(struct I2C_Info &i2c_info) {
    GPIO_InitTypeDef GPIO_InitStructure;
